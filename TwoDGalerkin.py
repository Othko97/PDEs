...
def generate_gal_mat_BC(n):
  ...
  dH = generate_dH(n)
  return np.array([([innerprod(dH[i], dH[j]) for j in range(1, n)]) for i in range(1, n)])

def find_rhs_BC(n, f):
  """Generate right hand side vector with u(0)=u(1)=0 applied"""
  H = generate_H(n)
  return np.array([[innerprod(f, H[i])] for i in range(1, n)])

def frac_rhs_BC(n, f):
  """Generate right hand side vector with u(0) = u(1) = 0 applied as integer ratios"""
  H = generate_H(n)
  return np.array([[(innerprod(f, H[i])).as_integer_ratio()] for i in range(1, n)])
